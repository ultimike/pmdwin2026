...
#[Block(
  ...
  admin_label: new TranslatableMarkup('My repositories stats'),
  category: new TranslatableMarkup('DrupalEasy'),
)]
final class MyRepositoriesStatsBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build(): array {
    $build['content'] = [
      ...
      '#items' => [
        ...
        $this->t('Total number of open issues in all repository nodes: @all', ['@all' => $this->calculateTotalIssues()]),
        $this->t('Total number of open issues in my repository nodes: @my', ['@my' => $this->calculateTotalIssues((int) $this->currentUser->id())]),
      ],
    ];
    $build['#cache'] = [
      'max-age' => 0,
  ...
  protected function calculateTotalIssues(int|null $uid = NULL): int {
    ...
    $query->condition('type', 'repository')
      ->condition('status', 1)
      ->condition('field_number_of_open_issues', 0, '>');
    if (!is_null($uid)) {
    ...
    foreach ($results as $nid) {
      ...
      if ($number_of_issues = $node->field_number_of_open_issues->value) {
